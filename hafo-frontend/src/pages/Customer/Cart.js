...
function Cart() {
    ...
    return (
        <div style={{ background: '#F7F2E5', minHeight: '100vh' }}>
            ...
            {/* Header phụ (đổi class để tránh dính CSS global .header) */}
            <header
            ...
            >
                    ...
                    <div className="items">
                        {cartItems.length === 0 ? (
                            <div style={{ padding: '40px', textAlign: 'center', color: '#888' }}>
                                ...
                                <Link to="/home" style={{ color: '#F97350', fontWeight: 'bold' }}>
                                    Đi chọn món ngay
                                </Link>
                            </div>
                        ) : (
                            cartItems.map((item) => (
                                ...
                                >
                                    ...
                                    <label className="i-check">
                                        <input type="checkbox" defaultChecked style={{ accentColor: '#F97350' }} />
                                    </label>
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            <strong style={{ minWidth: '18px', textAlign: 'center' }}>{item.quantity}</strong>
                                            <button
                                                onClick={() => updateQuantity(item.uniqueId, 1)}
                                                style={{
                                                    width: '28px',
                                    ...
                                    <div style={{ fontWeight: '800', minWidth: '84px', textAlign: 'right' }}>
                                        {toVND(item.finalPrice * item.quantity)}đ
                                    </div>
                                </div>
                            ))
                        )}
                ...
                >
                    <div style={{ display: 'flex', justifyContent: 'space-between', margin: '8px 0' }}>
                        <div style={{ color: '#6e655d', fontSize: '14px' }}>Số món đã chọn</div>
                        <div>{cartItems.reduce((acc, item) => acc + item.quantity, 0)}</div>
                    </div>

                    <div style={{ display: 'flex', justifyContent: 'space-between', margin: '8px 0' }}>
                        <div style={{ color: '#6e655d', fontSize: '14px' }}>Tạm tính</div>
                        <div style={{ fontWeight: '900', fontSize: '18px' }}>{toVND(totalAmount)}đ</div>
                    </div>

                    <div style={{ height: '1px', background: '#f0e8d9', margin: '10px 0' }} />
                    ...
                    <Link to="/checkout" style={{ textDecoration: 'none' }}>
                        <button
                            ...
                            style={{
                                width: '100%',
                                marginTop: '10px',
                                background: cartItems.length === 0 ? '#ccc' : '#F97350',
                                border: 'none',
                                color: '#fff',
                                padding: '12px 14px',
                                borderRadius: '12px',
